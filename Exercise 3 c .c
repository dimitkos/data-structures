...
{
	int i,mathima;
	int math[3];                           // dilosi pinaka 4 thesewn opou apothikeyoyme toys counters apo ta mathimata
	for (i=0; i<4; i++)                    
		math[i]=0;                          // arxikopoisi me 0 olwn twn thesewn toy pinaka
	
	for (i=0; i<N; i++)
	{
		mathima=students[i].lesson;         // apothikeysis sth thesi toy pinaka ton counter
		math[mathima]++;					// ayksisi kata 1 se periptosi epilogis toy xristi
	}
	
	printf("\n   Lessons   \n");
	printf("\n  Math [%d]\n",math[0]);
	printf("  Data Structures [%d]\n",math[1]);
	printf("  C/C++ Programming [%d]\n",math[2]);
	printf("  Compilers [%d]\n",math[3]);
}
